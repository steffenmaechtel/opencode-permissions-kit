# shellcheck shell=sh
# opencode permissions kit — shared UI helpers (ui.sh)
#
# One visual language for every kit script (install/update/config/status).
# POSIX sh, zero dependencies. Style decisions are recorded in
# docs/_archive/design/ux-improvement.md: labeled log lines (info/success/warn/error),
# slim banner, Unicode symbols with ASCII fallback.
#
# Rules:
#   - Colors are resolved to REAL bytes once at load time (printf only
#     interprets \033 in the format string, never in %s arguments).
#   - Colors off when NO_COLOR is set (https://no-color.org) or stdout is
#     not a tty — piping output into logs stays clean.
#   - UI_ASCII=1 forces plain ASCII fallbacks for broken locales.
#   - ui_ask/ui_menu print their prompt to stderr and the answer to stdout,
#     so $(...) capture stays clean; they read from /dev/tty when possible
#     (stdin may be the curl|bash script stream).
#
# Sourced (never executed):
#   . /usr/local/lib/opencode-permissions-kit/ui.sh

# --- colors ---------------------------------------------------------------------
if [ -n "${NO_COLOR:-}" ] || [ ! -t 1 ]; then
    UI_GREEN=''; UI_RED=''; UI_YELLOW=''; UI_CYAN=''; UI_BLUE=''
    UI_DIM=''; UI_BOLD=''; UI_NC=''
else
    UI_GREEN=$(printf '\033[0;32m'); UI_RED=$(printf '\033[0;31m')
    UI_YELLOW=$(printf '\033[0;33m'); UI_CYAN=$(printf '\033[0;36m')
    UI_BLUE=$(printf '\033[0;34m')
    UI_DIM=$(printf '\033[2m'); UI_BOLD=$(printf '\033[1m'); UI_NC=$(printf '\033[0m')
fi

if [ -n "${UI_ASCII:-}" ]; then
    UI_SYM_OK='ok'; UI_SYM_BAD='x'; UI_SYM_WARN='!'; UI_SYM_INFO='-'
    UI_SYM_ADD='+'; UI_SYM_ARROW='->'; UI_SYM_RULE='-'
else
    UI_SYM_OK='✔'; UI_SYM_BAD='✖'; UI_SYM_WARN='⚠'; UI_SYM_INFO='ℹ'
    UI_SYM_ADD='+'; UI_SYM_ARROW='→'; UI_SYM_RULE='─'
fi

# --- process log lines ------------------------------------------------------------
# info     Checking Node.js...
# success  Node.js v24 found

_ui_label() {
    # _ui_label <color> <label> <message...>
    printf '  %s%s%s  %s\n' "$1" "$2" "${UI_NC}" "$3"
}

ui_info()    { _ui_label "$UI_BLUE"   'info   ' "$1"; }
ui_success() { _ui_label "$UI_GREEN"  'success' "$1"; }
ui_warn()    { _ui_label "$UI_YELLOW" 'warn   ' "$1"; }
ui_error()   { _ui_label "$UI_RED"    'error  ' "$1"; }

# Indented, dimmed detail line (belongs to the previous ui_* line).
ui_detail()  { printf '     %s%s%s\n' "$UI_DIM" "$1" "$UI_NC"; }

# --- sections & banner --------------------------------------------------------------

ui_section() {
    # Section rule:  ── Pre-flight ─────────────────
    _t="$1"
    printf '\n  %s%s── %s ──' "$UI_BOLD" "$UI_CYAN" "$_t"
    _i=$(( 40 - ${#_t} ))
    while [ "$_i" -gt 0 ]; do printf '%s' "$UI_SYM_RULE"; _i=$((_i - 1)); done
    printf '%s\n\n' "$UI_NC"
}

ui_banner() {
    # Slim banner.
    printf '\n  %s%s opencode permissions kit%s  %sv%s\n\n' \
        "$UI_BOLD" "$UI_CYAN" "$UI_NC" "$UI_DIM" "${1:-0.0.0}"
    printf '  %s%s%s\n\n' "$UI_DIM" "${2:-UID-separated agent hardening}" "$UI_NC"
}

# --- inventory / checklist -----------------------------------------------------------

_ui_item() {
    # _ui_item <color> <symbol> <label> <note>
    printf '  %s%s%s  %-30s %s\n' "$1" "$2" "$UI_NC" "$3" "$4"
}

ui_have()  { _ui_item "$UI_GREEN"  "$UI_SYM_OK"   "$1" "$2"; }  # present/ready
ui_add()   { _ui_item "$UI_CYAN"   "$UI_SYM_ADD"  "$1" "$2"; }  # will be added
ui_atten() { _ui_item "$UI_YELLOW" "$UI_SYM_WARN" "$1" "$2"; }  # needs attention
ui_miss()  { _ui_item "$UI_RED"    "$UI_SYM_BAD"  "$1" "$2"; }  # missing/blocked

# --- aligned key/value panels (status, summaries) --------------------------------------

ui_kv() {
    # ui_kv <key> <value> [color-for-value]
    printf '  %-14s %s%s%s\n' "$1" "${3:-$UI_NC}" "$2" "$UI_NC"
}

ui_kv_warn() {
    # Like ui_kv, but with a warn symbol — value column aligns with ui_kv
    # (2 + 1 sym + 2 + 11 label + 1 space = same offset as 2 + 14 + 1).
    printf '  %s%s%s  %-11s %s%s%s\n' \
        "$UI_YELLOW" "$UI_SYM_WARN" "$UI_NC" "$1" "$UI_YELLOW" "$2" "$UI_NC"
}

# --- plan list (numbered) ----------------------------------------------------------------

ui_plan() {
    # ui_plan <n> <text> [note]
    printf '    %s%d%s  %-42s %s\n' "$UI_DIM" "$1" "$UI_NC" "$2" "${3:-}"
}

# --- questions ------------------------------------------------------------------------------

ui_ask() {
    # ui_ask "Question?" "default"  -> prints the answer on stdout.
    # Prompt on stderr (clean $(...) capture). Reads /dev/tty when possible;
    # EOF/empty -> default.
    _q="$1"; _d="${2:-}"
    if [ -n "$_d" ]; then
        printf '  %s [%s] > ' "$_q" "$_d" >&2
    else
        printf '  %s > ' "$_q" >&2
    fi
    IFS= read -r _ans </dev/tty 2>/dev/null || IFS= read -r _ans || _ans=''
    [ -z "$_ans" ] && _ans="$_d"
    printf '%s\n' "$_ans"
}

ui_confirm() {
    # ui_confirm "Question?" <default y|n>  -> exit status 0 = yes, 1 = no.
    # Convention (docs/design/conventions.md): prompt shows [Y/n] / [y/N]
    # with the DEFAULT as the capital letter; Enter accepts the default;
    # y/yes/n/no accepted (case-insensitive); EOF/unknown -> default.
    _q="$1"; _d="${2:-n}"
    [ "$_d" = "y" ] && _hint="[Y/n]" || _hint="[y/N]"
    printf '  %s %s ' "$_q" "$_hint" >&2
    IFS= read -r _ans </dev/tty 2>/dev/null || IFS= read -r _ans || _ans=''
    _ans="$(printf '%s' "$_ans" | tr '[:upper:]' '[:lower:]')"
    [ -z "$_ans" ] && _ans="$_d"
    case "$_ans" in
        y|yes) return 0 ;;
        *)     return 1 ;;
    esac
}

ui_menu() {
    # ui_menu "Question?" <default-key> <"key|description"...> -> prints the chosen KEY.
    # Menu on stderr (clean $(...) capture). Reads /dev/tty when possible;
    # EOF/empty/unknown -> default key.
    _q="$1"; _d="$2"; shift 2
    printf '  %s\n' "$_q" >&2
    for _opt in "$@"; do
        _k=${_opt%%|*}; _desc=${_opt#*|}
        if [ "$_k" = "$_d" ]; then
            printf '    %s[%s]%s  %s\n' "$UI_CYAN" "$_k" "$UI_NC" "$_desc" >&2
        else
            printf '     %s  %s\n' "$_k" "$_desc" >&2
        fi
    done
    printf '  > ' >&2
    IFS= read -r _ans </dev/tty 2>/dev/null || IFS= read -r _ans || _ans=''
    _found=''
    for _opt in "$@"; do
        _k=${_opt%%|*}
        [ "$_ans" = "$_k" ] && _found=1
    done
    [ -z "$_found" ] && _ans="$_d"
    printf '%s\n' "$_ans"
}
